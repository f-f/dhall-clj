...
(defemit* emit-combine-types-expression :combine-types 'deep-merge emit-times-expression)
(defemit* emit-prefer-expression        :prefer        'merge      emit-combine-types-expression)
...
(defn emit
  "Given an instaparse tree that is typechecked and normalized,
  emit a Clojure form."
  [expr]
  ...
  (let [transform-map {:complete-expression    emit-complete-expression
    ...
    (insta/transform transform-map expr)))
